...
export default function ManageVolt() {
    ...
    // Cleanup recently deleted IDs on unmount
    useEffect(() => {
        return () => {
    ...
    useEffect(() => {
        if (fetchedPresentations) {
            // Filter out deleted presentations and recently deleted presentations
            const activePresentations = (fetchedPresentations as PresentationData[]).filter(
                (presentation) =>
                    presentation.status !== 'DELETED' &&
                    !recentlyDeletedIds.has(presentation.id)
            );

            // Log any presentations that are still showing as DELETED but not in our recently deleted set
            const stillDeleted = (fetchedPresentations as PresentationData[])
                .filter(p => p.status === 'DELETED' && !recentlyDeletedIds.has(p.id));

            if (stillDeleted.length > 0) {
                console.warn('[ManageVolt] Found presentations still marked as DELETED from backend:', stillDeleted);
            }
            ...
            console.log('[ManageVolt] Filtered presentations:', {
                ...
                recentlyDeleted: recentlyDeletedIds.size,
                stillDeletedFromBackend: stillDeleted.length,
    ...
    }, [fetchedPresentations, recentlyDeletedIds]);
    ...
    const executeDeletePresentation = async () => {
        ...
        try {
            ...
            // Add to recently deleted set to prevent reappearance
            setRecentlyDeletedIds(prev => new Set([...prev, presentationToDelete.id]));
            ...
            // Clear any localStorage data for this presentation
            if (typeof window !== 'undefined') {
                // Clear slides data from localStorage
                localStorage.removeItem('slides');

                // Clear any Excalidraw data for this presentation
                const keysToRemove = [];
                for (let i = 0; i < localStorage.length; i++) {
                    const key = localStorage.key(i);
                    if (key && key.includes(presentationToDelete.id)) {
            ...
            // Clear the recently deleted ID after a delay
            setTimeout(() => {
                setRecentlyDeletedIds(prev => {
                    const newSet = new Set(prev);
                    newSet.delete(presentationToDelete.id);
                    return newSet;
                });
