...
const form = useForm<z.infer<typeof adminProfileSchema>>({
  ...
  const handleSubmitEditDataMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('Your details have been updated successfully!', { className: 'success-toast', duration: 2000 });
      setOpen(false);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger>
        <MyButton type="submit" scale="large" buttonType="secondary" layoutVariant="default" className="text-sm">
          Edit Profile
        </MyButton>
      </DialogTrigger>

      <DialogContent className="flex h-4/5 w-1/3 flex-col p-0">
        <h1 className="rounded-t-lg bg-primary-50 p-4 font-semibold text-primary-500">Edit Profile</h1>
                ...
                <div className="flex flex-col items-center justify-center gap-8">
                  <div className="relative">
                    {form.watch('profilePictureUrl') ? (
                      <img src={form.watch('profilePictureUrl')} alt="logo" className="size-52 rounded-full object-cover" />
                    ) : (
                    ...
                    <div className="absolute bottom-3 right-3 z-10">
                      <Menu as="div" className="relative inline-block text-left">
                        ...
                        <Transition>
                          <Menu.Items className="absolute bottom-10 right-0 z-20 w-40 origin-bottom-right bg-white rounded-md shadow-lg ring-1 ring-black/5">
