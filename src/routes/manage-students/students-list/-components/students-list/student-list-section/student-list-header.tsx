...
import { useAllBatchIds, useBatchesByIds } from '@/services/paginated-batches';
...
const InviteLinksDialog = ({
    ...
    const batchIds = useMemo(
        () => legacyData?.flatMap((group) => group.batches.map((b) => b.package_session_id)) || [],
        [legacyData]
    );

    const { data: detailsData, isLoading: detailsLoading } = useBatchesByIds(batchIds);

    const batchesData = useMemo(() => {
        if (!legacyData) return { content: [] };

        const detailsMap = new Map();
        if (detailsData?.content) {
            detailsData.content.forEach((b) => detailsMap.set(b.id, b));
        }

        const flatList = legacyData.flatMap((group) =>
            group.batches.map((b) => {
                const detail = detailsMap.get(b.package_session_id);
                return {
                    id: b.package_session_id,
                    batch_name: b.batch_name,
                    invite_code: b.invite_code,
                    package_dto: group.package_dto,
                    level: detail?.level,
                    session: detail?.session,
                };
            })
        );
        return { content: flatList };
    }, [legacyData, detailsData]);

    const isLoading = legacyLoading || (batchIds.length > 0 && detailsLoading);
        ...
        >
            {isLoading ? (
                ...
                <div className="flex max-h-[60vh] flex-col gap-2 overflow-y-auto">
                    {batchesData?.content && batchesData.content.length > 0 ? (
                        batchesData.content.map((batch, index) => {
                            const courseName = convertCapitalToTitleCase(
                                batch.package_dto.package_name
                            );
                            const levelName = batch.level?.level_name || 'Unknown Level';
                            const sesName =
                                batch.session?.session_name || currentSession?.name || '';
                                    ...
                                    <div className="flex items-start justify-between gap-2">
                                        <div className="flex-1">
                                            <div className="mb-1.5 flex items-center gap-1.5">
                                                ...
                                                <h3 className="text-xs font-semibold text-primary-600 transition-colors duration-200 group-hover:text-primary-700">
                                                    {batch.batch_name}
                                                </h3>
                                            </div>
...
export const StudentListHeader = ({
    ...
    // Check if any batches exist using paginated API
    const { data: allBatchIds } = useAllBatchIds();
    const hasBatches = allBatchIds && allBatchIds.length > 0;
                        ...
                        <NoCourseDialog
                            ...
                            trigger={
                                <MyButton
                                    scale="small"
                                    className={cn(
                                        'hover:scale-102 group flex items-center gap-1 border-0 bg-primary-600 text-white shadow-sm transition-all duration-200 hover:bg-primary-700 hover:shadow-md',
                                        isCompact
