...
const formSchema = z.object({
  ...
  videoName: z.string().optional(), // ✅ Add videoName here
});
...
export const AddVideoDialog = ({ openState }: { openState?: (open: boolean) => void }) => {
  ...
  const submitFormWithDuration = async (data: FormValues, duration: number) => {
    ...
    try {
      ...
      const title = data.videoName || 'YouTube Video';
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="flex w-full flex-col gap-6 p-6 text-neutral-600">
            ...
            <div className="rounded-xl border bg-neutral-50 p-4 duration-500 animate-in fade-in slide-in-from-bottom-2">
              <div className="flex items-center gap-3">
                <div className="relative flex-shrink-0">
                  <img src={videoPreview.thumbnail} alt="Video thumbnail" className="h-12 w-16 rounded-lg object-cover" />
                  <div className="absolute inset-0 flex items-center justify-center">
                    <PlayCircle className="h-6 w-6 text-white drop-shadow-lg" />
                  </div>
                </div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-neutral-700">{videoPreview.title}</p>
        ...
        <div className="flex justify-end border-t border-neutral-100 pt-4">
          {isVideoUploading ? (
            <MyButton type="button" buttonType="primary" scale="large" layoutVariant="default" className="w-full">
            ...
            <MyButton
              ...
              disabled={!form.getValues('videoUrl') || !isValidUrl}
              className={`w-full transition-all duration-300 ease-in-out ${
                !form.getValues('videoUrl') || !isValidUrl
