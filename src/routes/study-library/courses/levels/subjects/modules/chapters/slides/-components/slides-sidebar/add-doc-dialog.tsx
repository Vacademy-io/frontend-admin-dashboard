...
export const AddDocDialog = ({ openState }: { openState?: (open: boolean) => void }) => {
  ...
  const { courseId, levelId, chapterId, moduleId, subjectId, sessionId } = route.state.location.search;
      ...
      <form onSubmit={form.handleSubmit(handleUpload)} className="flex flex-col gap-6 p-6">
        ...
        {isUploading && (
          <div>
            ...
            />
            <p className="mt-2 text-sm text-neutral-600">This may take a few moments...</p>
          </div>
        )}
        ...
        <DialogFooter className="flex w-full items-center justify-center">
          <MyButton
            ...
            className="w-full"
          >
            {isUploading ? 'Upload...' : 'Upload Document'}
          </MyButton>
        </DialogFooter>
      </form>
    </Form>
